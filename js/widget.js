...
var widget = (function(){
  ...
  var self = {
    ...
    generateChatPanel: function generateChatPanel( selector, options )
    {
      var chatPanel = {
        options: options,
        view: $('<div/>').addClass( 'chatPanel' ),
        addMessage: function( message, sender, createdTime, autoScroll ) {
          createdTime = createdTime || Date.now();
          sender = sender || {};

          var messageEntry = $('<div/>').addClass('messageEntry');
          messageEntry.addClass( (sender.key == options.senderContact.key) ? 'local' : 'remote' );

          widget.makeAutorefresh( $('<div/>'), function( view ) {
            view.text( moment(createdTime).fromNow() );
          }, 2000, true ).addClass('timestamp').appendTo( messageEntry );

          var holder = $("<div/>").addClass( 'chatThumbnailHolder' ).appendTo( messageEntry );
          $("<div/>").css( 'background-image', 'url('+ sender.thumbnail +')' ).addClass( 'sender' ).appendTo( holder );

          if( Autolinker !== undefined && $.isFunction( Autolinker.link ) )
            message = Autolinker.link( message );

          var messageView = $('<div/>', {html: message } ).addClass('message').appendTo( messageEntry );
          if( $.isFunction( messageView.emoticonize ) )
            messageView.emoticonize();

          var content = $('.chatContent', chatPanel.view);
          content.append( messageEntry );
          if( autoScroll )
            chatPanel.makeRecentVisible( true );
        },
        makeRecentVisible: function( animated ) {
          var content = $('.chatContent', chatPanel.view);
          var duration = animated ? 400 : 0;

          content.animate( { scrollTop: content[0].scrollHeight}, duration );
        }
      };

      $('<div/>').addClass( 'chatContent' ).appendTo( chatPanel.view );
      var controls = $('<div/>').addClass( 'controls' ).appendTo( chatPanel.view );
      var inputField = $('<input/>').attr( {type:'text', placeholder:'Message'}).appendTo( controls );
      ...
      var sendButton = $('<button/>', {
        ...
        click: function() {
          ...
          {
            inputField.val('');
            var suppressLocalPost = false;
            if( chatPanel.options.sendMessage )
              suppressLocalPost = chatPanel.options.sendMessage( msg );

            if( !suppressLocalPost )
              chatPanel.addMessage( msg, options.senderContact, undefined, true );
          }
        }
      } ).appendTo( controls );

      // Bind textinput's enter key to the button's click
      inputField.on( 'enterKey', function(e){ sendButton.trigger( 'click' ); } );
      inputField.keyup(function(e) { if(e.keyCode == 13) { $(this).trigger( 'enterKey' ); } });

      $(selector).append( chatPanel.view );
      return chatPanel;
    },

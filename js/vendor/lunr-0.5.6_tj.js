...
lunr.stemmer = (function(){
  ...
  var memo = {};
  var memoizingStemmer = function memoizingStemmer(w ) {
    if (!memo[w]) {
      //memo[w] = porterStemmer( w );
      memo[w] = lancasterStemmer( w );
    }
    return memo[w];
  };

  return memoizingStemmer;
})();
...
lunr.trimmer = function (token) {
  return token
    .replace(lunr._re_trim1, '')
    .replace(lunr._re_trim2, '')
}
